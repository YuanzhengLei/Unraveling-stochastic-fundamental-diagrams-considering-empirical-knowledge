...
"""
n is the size of the inducing variable
m is the line index that used to record results in the exile file
'Weights.csv' stores weights for each density-speed pairs, which can be calculate based on weighted least square method proposed in {On the fundamental diagram for freeway traffic: A novel calibration approach for single-regime models}
reproduce code can be find in 'https://github.com/YuanzhengLei/Weighted-the-least-square-method-for-single-regime-fundamental-diagram-models'
"""

n = 288
m = 2
MaxSize = 500
file_path = r"C:\Users\user\PycharmProjects\GP\SizeTest.xlsx"
workbook = load_workbook(filename=file_path)
sheet1 = workbook['RMSE_CS']
sheet2 = workbook['MAPE_CS']
...
while n <= MaxSize :
    ...
    #n = math.floor(len(X_sorted)/100) # Number of samples you want to select
    ...
    sheet1.cell(row=m, column=1, value=n)
    sheet2.cell(row=m, column=1, value=n)
    ...
    sheet1.cell(row=m, column=2, value=rmse_result_PureGP)
    sheet2.cell(row=m, column=2, value=mape_result_PureGP)
    ...
    sheet1.cell(row=m, column=3, value=rmse_result_Cheng)
    sheet2.cell(row=m, column=3, value=mape_result_Cheng)
    ...
    sheet1.cell(row=m, column=4, value=rmse_result_Wang)
    sheet2.cell(row=m, column=4, value=mape_result_Wang)
    ...
    sheet1.cell(row=m, column=5, value=rmse_result_Greenshields)
    sheet2.cell(row=m, column=5, value=mape_result_Greenshields)
    ...
    sheet1.cell(row=m, column=6, value=rmse_result_Greenberg)
    sheet2.cell(row=m, column=6, value=mape_result_Greenberg)
    ...
    sheet1.cell(row=m, column=7, value=rmse_result_Underwood)
    sheet2.cell(row=m, column=7, value=mape_result_Underwood)
    ...
    sheet1.cell(row=m, column=8, value=rmse_result_Newell)
    sheet2.cell(row=m, column=8, value=mape_result_Newell)
    ...
    sheet1.cell(row=m, column=9, value=rmse_result_Pipes)
    sheet2.cell(row=m, column=9, value=mape_result_Pipes)
    ...
    sheet1.cell(row=m, column=10, value=rmse_result_Papageorgiou)
    sheet2.cell(row=m, column=10, value=mape_result_Papageorgiou)
    ...
    sheet1.cell(row=m, column=11, value=rmse_result_kerner_Konhauser)
    sheet2.cell(row=m, column=11, value=mape_result_kerner_Konhauser)
    ...
    sheet1.cell(row=m, column=12, value=rmse_result_DelCastillo_Benitez)
    sheet2.cell(row=m, column=12, value=mape_result_DelCastillo_Benitez)
    ...
    sheet1.cell(row=m, column=13, value=rmse_result_Jayakrishnan)
    sheet2.cell(row=m, column=13, value=mape_result_Jayakrishnan)
    n = n + 5
    m = m + 1
